...
With the introduction of a live review system, it is crucial that the system is available and functional at all times (or as close to as reasonable possible). Since live review data cannot be collected retrospectively, it is crucial that when the user wants/need to collect live data, they are able to - hence availability of the system has been elevated to be a more important quality requirement.
\subsection{Security}
...
\begin{itemize}
    \item Availability: The use of a Service-Oriented Architecture promotes availability of the application by distributing the computational responsibilities across different deployed units such that if a single unit fails, the remaining units may function (ie: live reviews may still be collected even if the profiles service or source connector fails).
